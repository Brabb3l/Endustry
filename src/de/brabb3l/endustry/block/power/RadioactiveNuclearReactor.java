package de.brabb3l.endustry.block.power;
...
public class RadioactiveNuclearReactor extends PowerGenerator {
    ...
    public RadioactiveNuclearReactor(String name){
        super(name);
        itemCapacity = 30;
        liquidCapacity = 30;
        hasItems = true;
    ...
    public void onDestroyed(Tile tile){
        ...
        for (int xx = -explosionRadius; xx < explosionRadius; xx++) {
        	 for (int yy = -explosionRadius; yy < explosionRadius; yy++) {
        		if (xx * xx + yy * yy <= explosionRadiusSquared && Mathf.chance(0.5f - 0.5f * ((xx * xx + yy * yy) / explosionRadiusSquared))) {
					float x = tile.x + xx;
					float y = tile.y + yy;
					
					Tile other = world.tile((int)x, (int)y);
					Radiation.create(other);
        		}
     		}
		}
    }
